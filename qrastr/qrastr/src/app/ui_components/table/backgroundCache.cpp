...
void BackgroundCache::invalidateColumn(ModelIndex col) {
	for (auto& [row, cols] : data)
		cols.erase(col);
}

const QVariant* 
BackgroundCache::get(int row, ModelIndex col) const {
...
void
BackgroundCache::put(int row, ModelIndex col, QVariant v) {
	data[row][col] = std::move(v);
}

void BackgroundCache::shiftRowsDown(int first, int count) {
    std::unordered_map<int, std::unordered_map<ModelIndex, QVariant>> shifted;
	for (auto& [row, cols] : data) {
